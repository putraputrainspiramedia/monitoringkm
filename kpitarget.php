	...
	<script language="javascript">
		...
		$(document).ready(function(){
			...
			$("#tambah").click(function () {
				...
				$("#organisasi2").val($("#organisasi").val());
				$("#hapus2").hide();
				$("#simpan").show();
				$('.modal-title').html('Input KPI Target');
			...
			$("#edit").click(function () {
           		window.location.href='index.php?fl=kpitarget&tp=edit';
       		});
			<?php } ?>
			...
			$("#excel").click(function () {
				window.location.href='kpitargetexcel.php?tahun='+$("#tahun").val()+'&organisasi='+$("#organisasi").val();
           	});
		});
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                	<form method="post" enctype="multipart/form-data" action="kpitargetdata.php">
                       ...
                       <div class="form-group">
                           ...
                           <select name="organisasi" id="organisasi2" class="form-control">
                            	<?php 
									$qry_pil = mysqli_query($conn,"select id_organisasi, nama_organisasi 
																from p_organisasi order by nama_organisasi");
									while ($dt_pil = mysqli_fetch_array($qry_pil)) {
...
<div class="col-md-12">
	<div class="card">
		<div class="card-header card-header-action">
			<h6>Input KPI Target</h6>
		</div>
		<div class="card-body">   
		<?php if ($menu_arr['input']==1) { ?>
			<form method="post" action="kpitargetdata.php" enctype="multipart/form-data">
...
function edit_data() {
	...
	if (!empty($_REQUEST['kpi']) and !empty($_REQUEST['periode']) and !empty($_REQUEST['tahun']) and !empty($_REQUEST['organisasi']) and !empty($_REQUEST['tahun'])  and !empty($_REQUEST['status'])) {
		$qry = mysqli_query($conn,"select target from kpi_targetbobot where id_kpi = '".$_REQUEST['kpi']."'
		...
		$target = number_format($dt['target'],2,",",".");							
	}
	?>
	<script>
		  $(document).ready(function(){
			...
			$("#simpan").click(function(){
				...
				$.ajax({
					...
					success	: function(data){
						alert(data);
						window.location.href='?fl=kpitarget';
					}
				});
			});
						...
						<div class="col-md-4 form-group">
                         ...
                         <div class="col-md-4 form-group">
                        	<label style="color:#FF0000">misal 1.234,56 (2 digit dibelakang koma)</label>
							 <input type="text" class="form-control angka" id="bobot" placeholder="target" name="target" maxlength="10"  value="<?=$target;?>">
						</div>  
					</div>
				</div>
			</div>
