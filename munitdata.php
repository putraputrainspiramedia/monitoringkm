...
function list_data(){
	...
	if (!empty($searchVal)) {
		$where = " and (nama_unit like '%".$searchVal."%') ";
	}
	
	$stmt = "select a.id_unit, a.nama_unit, b.nama, DATE_FORMAT(a.tgl_input,'%d-%m-%Y') tgl_input
			from p_unit a
			left join user_app b on a.id_user_input = b.id_user
				where a.id_unit is not null ".$where."
				ORDER BY $orderCol $orderDir
				limit $start, $limit";
			
	$stmt_count = "select id_unit from p_unit 
				where id_unit is not null ".$where."";
	...
	if($query){
		...
		while ($row = mysqli_fetch_array($query)){
				...
				#$data[$i]["action"] = "<a href='?fl=munit&tp=edit&id=".enkripsi($row['id_unit'])."'><img src='img/pencil-icon.png' title='Ubah'></a>&nbsp;&nbsp;&nbsp;<a href='?fl=munit&tp=delete&id=".enkripsi($row['id_unit'])."'><img src='img/cross-icon.png' title='Hapus'></a>";
				$data[$i]["action"] = "<a href='#'  onClick='editdata(\"".enkripsi($row['id_unit'])."\",\"".$row['nama_unit']."\");'><img src='img/pencil-icon.png' title='Ubah' id='edit'></a>&nbsp;&nbsp;&nbsp;<a href='#' onClick='hapusdata(\"".enkripsi($row['id_unit'])."\",\"".$row['nama_unit']."\");'><img src='img/cross-icon.png' title='Hapus' id='hapus'></a>";
			$i++;
		}
...
function input_data(){
	...
	if ($msg==''){	
		$stm = "insert into p_unit (nama_unit, tgl_input, id_user_input, ip_input) 
	...
	echo $msg;
	/*
	echo "<script>alert('".$msg."');window.location.href='index.php?fl=munit'</script>";*/
}
...
function edit_data(){	
	...
	if ($msg==''){
		...
		$qry = mysqli_query($conn,"update p_unit set nama_unit = '".$_POST['nama']."',
...
function delete_data(){
	...
	$qry = mysqli_query($conn,"delete from p_unit where id_unit = '".dekripsi($_REQUEST['id'])."'");
	if($qry){
		$msg = "sukses";
	}else{
		$msg = "gagal";
