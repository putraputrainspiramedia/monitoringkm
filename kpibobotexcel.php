...
$objPHPExcel->getProperties()->setCreator("Andex Teddy / exnome@gmail.com")
							 ...
							 ->setTitle("KPI Bobot")
							 ->setSubject("KPI Bobot")
							 ->setDescription("KPI Bobot")
							 ->setKeywords("KPI Bobot")
							 ->setCategory("KPI Bobot");
...
$objPHPExcel->getActiveSheet()->setTitle('KPI Bobot');
...
while ($row_bsc = mysqli_fetch_array($stmt_bsc)) {			
	...
	for($ii=0;$ii<$jml_periode;$ii++) { 
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i, $j, $row_bsc['nilai']);
		$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($i)->setAutoSize(true);		$i++;
	}
...
for($ii=0;$ii<$jml_periode;$ii++) { 
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($i, $j, '100');
	$objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($i)->setAutoSize(true);		$i++;
}	


$tgl = date("Ymd_His");
$filename = "kpibobot".$tahun;
