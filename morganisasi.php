...
?>
	...
	<script language="javascript">
		...
		function hapusdata(id,nama, idreg, idgroup){
			$("#hapus2").show();
			$("#simpan").hide();
			$("#id").val(id);
			$("#nama").val(nama);
			...
			$('.modal-title').html('Hapus Master Organisasi');
		...
		function editdata(id,nama, idreg, idgroup){				
			$("#hapus2").hide();
			$("#simpan").show();
			$("#id").val(id);
			$("#nama").val(nama);
			...
			$('.modal-title').html('Edit Master Organisasi');
			$('#InputModalForms').modal('show');
		...
		$(document).ready(function(){
			...
			$("#tambah").click(function () {
				...
				$("#hapus2").hide();
				$("#simpan").show();
				$('.modal-title').html('Input Master Organisasi');
           		$('#InputModalForms').modal('show');
       		});
			<?php } ?>
			...
			$("#simpan").click(function () {
				...
				if (id!='') {
					var datane = 'tp=edit&nama='+nama+'&id='+id+'&regional='+regional+'&group='+group;
				} else {
					var datane = 'tp=input&nama='+nama+'&regional='+regional+'&group='+group;
				}
				$.ajax({
					type	: "POST",
					url		: "morganisasidata.php",
                       ...
                       <div class="form-group">
                            <label>Group</label>
                                <select name="group" id="group" class="form-control">
                                    ...
                                    <?php 
                                        $qry_pil = mysqli_query($conn,"select id_group, nama_group from p_group order by nama_group");
                                        while ($dt_pil = mysqli_fetch_array($qry_pil)) {
                                            echo "<option value='".$dt_pil['id_group']."'>".$dt_pil['nama_group']."</option>";
                                        }
                                    ?>
                                </select>
                         </div>
